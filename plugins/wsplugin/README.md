...
실행 중 개발 서버의 기본 JAR을 건드리지 않고 별도 후보를 만들 때는 다음처럼 분류명을 지정한다.

```powershell
.\gradlew.bat shadowJar -PwsArtifactClassifier=candidate -PrunServerPort=25566
```

이 경우 결과는 `build/libs/wsplugin-0.0.1-ALPHA-candidate.jar`이며, 별도 포트 스모크에만 사용한다. 운영 JAR 교체는 기존 서버를 `/stop`으로 정상 종료한 뒤 수행한다.
...
| 무기 스킬 W2 | `Shift+L` |
| 무기 스킬 W3 | `Shift+R` |
| 짧은 패링/단기 방어 | 보조 방패 장착 후 슬롯 `0`에서 일반 `F` 1회 |
| 공용 액티브 C1~C4 | 슬롯 `0` 전투 자세에서 `Shift+2~5` |
| 퀵 아이템 Q1~Q4 | 슬롯 `0` 전투 자세에서 `Shift+6~9` |
...
보조 방패를 장착한 상태의 슬롯 `0` 일반 `F`는 키 홀드·해제·토글 없이 `1틱 준비→4틱 패링→12틱 단기 방어→자동 종료`한다. 공격·스킬·회피·슬롯 이탈·방패 파손·강한 행동 불가는 현재 실행을 취소하며, Shift+F 메뉴와 슬롯 `1~8`의 바닐라 손 교환은 이 상태기계를 시작하지 않는다.

slot `0` 곡괭이로 허용 블록을 채굴하면 Bukkit `PlayerItemDamageEvent`는 취소하지 않는다. 플러그인은 적용 damage를 `0`으로 중화해 ItemStack 삭제를 막고 서버 내구 원장을 한 번 차감한다. 원장 내구가 `0`이면 장비를 `BROKEN`으로 보존하고 권위 `EquipmentBrokenEvent`와 Bukkit 호환 `PlayerItemBreakEvent`를 `instanceId+파손 차수` 멱등 키로 전이당 한 번씩 발행한다. slot `0`은 명시적으로 `MAIN_HAND`으로 판정하며 호환 이벤트용 파손 직전 ItemStack을 만들 수 없으면 전이를 커밋하지 않는다. 성공 시 주손 파손 효과·사운드를 보내고 다음 틱 서버 미러를 복원한다. 실제 장착 아이템은 삭제하지 않으며 바닐라 아이템은 기존 이벤트와 소멸 동작을 그대로 사용한다.

## 기본 플레이어 속성·빈사

- 기본 이동속도는 바닐라의 `110%`다. SPD 투자는 포인트당 바닐라 기준 `+0.5%p`를 추가한다.
- 블록 상호작용 거리는 `5.85블록`, 개체 상호작용 거리는 `3.9블록`으로 바닐라 대비 `+30%`다.
- 무기·스킬·구조처럼 자체 사거리가 있는 판정은 위 상호작용 거리로 늘어나지 않는다. 구조 거리는 `2.5블록`을 유지한다.
- 생존 플레이어의 치명 피해는 최종 피해 보정 뒤 가로채 HP `1`의 빈사 상태로 전환한다. 빈사 중 전투·아이템·블록·GUI·아이템 이동·질주·점프·다른 플레이어 구조는 차단한다.
- 프로토타입은 현재 빈사 후 `30초` 구조 제한시간을 사용한다. 정식 별도 빈사 체력·부상 스택·빈사 피해는 `DEATH-001~003` 후속 범위다.
...
- 자동 빌드·L0 콘텐츠 검증·순수 도메인 단위 테스트: 2026-08-24 통과
- Paper 26.1.2 격리 기동·플러그인 활성화·18단계 Advancement 등록·런타임 L0 검증·명령 등록·정상 종료: 2026-08-24 최신 보정 JAR 스모크 통과
- 프로덕션 카탈로그 70파일·334 도감·214 장비·315 조합법·64 스킬·66 증강·21 상태·46 시설·91 엔티티·62 전리품: 자동 검증 통과
- 정식 적 53종의 기본 전조 행동과 개인 전리품 보상함은 Test Lab에서 호출 가능한 준비 상태다. 일반 `PROTOTYPE` 회차의 Day 진행에는 연결하지 않았으며, 전체 보스 패턴·지원 개체·Day 사건 완료를 뜻하지 않는다.
...
- 블록 설치·slot0 곡괭이 채굴은 실제 클라이언트 입력이 필요한 `BLOCKED_E2E`다. `/setblock`은 이 통과 판정을 대신하지 않는다.
