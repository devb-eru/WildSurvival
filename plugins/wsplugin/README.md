...
| 퀵 아이템 Q1~Q4 | `Shift+6~9` |
| 통합 플레이어 메뉴 | `Shift+F` |
| 회피 보조 입력 | 빠른 `Shift` 두 번 |

전투 입력은 현재 선택 슬롯이 `0`일 때만 해석한다. 주무기는 실제 핫바 슬롯 `0`, 보조무기는 오프핸드 `-106`에 서버가 동기화하며 스킬·공용 액티브·퀵 아이템 실행 뒤 슬롯 `0`으로 돌아간다. 슬롯 `1~8`은 채굴·설치·섭취를 포함한 바닐라 자유 슬롯이다. 권투를 제외한 장착 무기는 바닐라 피해를 사용하지 않으며, 두 장착 슬롯이 비면 권투 가상 프로필을 사용한다.
...
- 유틸리티: 야전 화살·수지 횃불·공용 보급 저장소
- 3×3 고정 조합법 15종, 무기 스킬 13종

등록 자원 블록은 바닐라 드롭과 WS 드롭을 동시에 주지 않는다. 석재·연료탄은 급조 곡괭이 이상, 철은 석재 곡괭이 이상이 필요하다. Craft 입력 3×3은 흰 유리, 사용하지 않는 영역은 회색 유리로 구분한다.
...
- Paper 26.1.2 기동·플러그인 활성화·18단계 Advancement 등록·런타임 L0 검증·명령 등록·정상 종료: 2026-08-23 스모크 통과
