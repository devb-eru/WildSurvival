...
| 진행 예비 | `PROGRESSION_RESERVE_DAY40_FINAL`; 제작 후 현재 보유+남은 보장 공급으로 Day40 호출과 Final 최소 재건 경로를 모두 충족할 수 없으면 제작 예약 거부 |
...
진행 예비 검사는 코어를 Final 선행으로 만들기 위한 규칙이 아니다. 선택 소모품 때문에 필수 호출·재건이 영구 잠기는 것을 막는 안전선이다. 현재 자원, 이미 예약된 작업, 각 Day의 아직 수령 가능한 보장 공급을 ID별로 합산하고 `BLOCKED_PROGRESSION_RESERVE`일 때 부족 ID·수량을 GUI에 표시한다.
