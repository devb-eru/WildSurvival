...
비장비 아이템 생성기는 `item-s1-r2`의 카테고리별 행 계약을 사용한다. 종전처럼 소모품 열 위치를 휴대 장치·시설 키트·호출품에 재사용하지 않으며, 61개 모두 `textKey/customModelKey/ownership/usePolicy/recipeId`를 가진다. 데이터·스키마·L0 회귀 검증은 통과했지만 탄약 원장·휴대 장치 고유 효과·ArenaManifest·호출 트랜잭션은 `ITEM-RUNTIME-STATUS-001`에 따라 아직 미완료다.
