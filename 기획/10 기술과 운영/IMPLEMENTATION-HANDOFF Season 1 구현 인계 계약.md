...
현재 인계는 `IMPLEMENTATION_IN_PROGRESS`다. r2 70파일과 L0 validator, 21개 상태 권위, slot0 중재·서버 내구도·BROKEN, 10개 무기군, 214개 장비의 슬롯 정규화와 GUI 장착·동기화, 334칸 생산 도감 로더까지 구현됐다. 2026-08-23 Paper 26.1.2 빌드 74에서 플러그인 활성화, 런타임 검증 명령, 정상 비활성화와 월드 저장을 통과했다. 전체 315개 레시피의 정확 입력 실행, 생산 장비 고유 효과·64개 스킬·66개 증강·21개 상태·91개 엔티티·46개 시설의 의미 런타임, Paper 멀티플레이·부하 테스트는 아직 완료로 표시하지 않는다. 실제 클라이언트가 필요한 블록 설치·채굴 판정은 `BLOCKED_E2E`이며 `/setblock` 결과로 대체 승인하지 않는다.
