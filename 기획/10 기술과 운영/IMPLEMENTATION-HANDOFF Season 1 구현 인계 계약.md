...
관리자 보급 없는 `E2E-20` 새 회차의 팀 시작 기준점은 오버월드 월드 스폰으로 확정됐다. 현재 런타임은 로비 위치를 그대로 쓰고 `RULES` §7.1의 12개 후보·안전 경로·원목/노출 석재/식량 확인·실패 Manifest를 실행하지 않으므로 시작 소프트락 방지 인계는 미완료다. `CRAFT-001`의 등록 목재 1=원목 4개와 현 채집의 블록당 `WSR-WOOD` 1개도 단위 불일치로 추적한다. 둘 다 Day 1~3 무보정 실플레이 완료 전에 닫아야 한다.
