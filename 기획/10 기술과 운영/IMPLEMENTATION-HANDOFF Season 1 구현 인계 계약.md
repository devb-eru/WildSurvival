...
| `E2E-17` | 자연 원목·광물 채집, 개인 정규화와 도구 등급 | 허용 바닐라 원자재 유지, 같은 노드의 등록 자원 무상 중복 지급 0, Craft에서 원목 4→목재 1·석재 8→등록 석재 1, 부족 도구 등급은 블록 보존 |
...
P2 구현 후보는 개인 PDC 자원과 회차 개인 원장을 단일 권위로 통합하고, 연구·시설 비용 예약/취소와 FAC-S16 입출금을 물리 수량 체크포인트까지 연결했다. Craft 해금의 원목 4개, 3×3 제작 입력 예약, 고정 출력 서명, 일반 아이템 목표 수량, 자원 원장 출력, 장비 고정 instanceId 지급, 저장소 아이템 소비·시설 상태를 각각 내구 우선·물리 후반영 순서로 전환했다. 미완료 `CRAFT` 거래는 소유자 접속 시 같은 출력 서명으로 재개한다. Test Lab은 잠금 해금 전용 `CRAFT-UNLOCK`, 거래용 `CRAFT`, 최신 제작 거래 검사와 `RESERVED/PROCESSING/COMMITTED` 단발 중단점을 제공하며 해금 원목은 전투 전용 slot 0을 사용하지 않는다. 자동 검증은 51 suite·185 test와 세 콘텐츠 번들 검증을 통과했고 배포 후보 JAR SHA-256은 `2C13E408A44C7217011F821276F16FF02DD77859AD373ACF54602643752020FE`다. `E2E-14~20` 및 실제 JVM 중단 증거가 없으므로 P2는 `IMPLEMENTED_AWAITING_CLIENT_EVIDENCE`, 전체 인계는 계속 `IMPLEMENTATION_IN_PROGRESS`다.

새 Test Lab 진입 후 동결 상태에서 Day 2로 조기 진행하는 결함은 Day 초기 상태의 시작·공세 시각·위협 예산 0에서 비롯됐다. 교체 스냅샷의 Day 1 상태와 수동 Day 선택의 인원별 예산을 동일 정책으로 생성하는 로컬 보정 및 52 suite·188 test가 통과했다. 로컬 후보 JAR SHA-256은 `FB77726FA7316D66D8B42A05AF80FD59702A0B7AA1F7D869CF863434BCF7689F`다. 기존 배포 JAR은 아직 이 보정 전이므로 실클라이언트의 `world day set 1` 임시 경로로 진행하되, P2 Day 1 무보정 진입 증거는 다음 후보 배포·재입장 뒤에만 승격한다.

P2 해금 입력은 `non2error`의 실제 GUI에서 성공 문구·원목 잔량 0·3×3 칸 개방까지 통과했다. 회차의 해금 키는 1개이고 실물 원목 체크포인트는 비었다. 같은 회차의 `CRAFT` 시나리오에서 급조 곡괭이 고정 조합과 실제 1개 지급, 목재 `50→47`·섬유 `50→48`, 단일 `CRAFT/COMMITTED` 거래 및 미완료 체크포인트 0을 확인했다. 목재 1개를 조합 칸에 넣은 뒤 Esc로 닫은 반환도 실제 목재 47개·드롭 0·신규 거래 0으로 통과했다. 구 배포 JAR의 해금·제작 성공 통계 중복은 각각 outbox 1개와 별개이며, 로컬 `3cda3d3`은 `CRAFT_UNLOCKED`와 `CRAFT_COMMITTED`를 내구 커밋 outbox에서만 발행하고 후자에 거래 ID를 싣는다. 자동 검증은 52 suite·188 test 무실패, 새 로컬 JAR SHA-256은 `3AB1F24B065AF1A9C5007D10E76C6590EE7FAE5EAA1E59818F790AC4C72D50C1`다. 이 후보는 접속 중인 격리 서버에 아직 배포하지 않았으며, P2 전체 게이트는 제작 중단 복구·무보정 Day 진입과 다인 루프 전까지 열린다.

관리자 보급 없는 `E2E-20` 새 회차의 팀 시작 기준점은 오버월드 월드 스폰으로 확정됐다. 현재 런타임은 로비 위치를 그대로 쓰고 `RULES` §7.1의 12개 후보·안전 경로·원목/노출 석재/식량 확인·실패 Manifest를 실행하지 않으므로 시작 소프트락 방지 인계는 미완료다. 등록 목재는 사용자가 `원목 4개=WSR-WOOD 1개`로 확정했다. 바닐라 원목은 보유·건축에 남고 개인 `Craft GUI → 자원 정규화`로 등록 목재를 만든 뒤에만 개인 제작·공용 입출금에 사용한다. 현 채집의 블록당 `WSR-WOOD` 1개, 초반 비용·석재/섬유 단위는 이 계약에 맞춰 수정해야 하며 Day 1~3 무보정 실플레이 전까지 미완료다.

`P2-CRAFT-RESERVED-KILL-001` 실제 JVM 중단은 `RESERVED` 단일 목재 3·섬유 2 예약 직후 Java PID 29776을 종료 훅 없이 강제 종료하고 동일한 배포 JAR로 재기동해 검증했다. 접속 전 `RESERVED/목재 44/섬유 46/거래 2건`, `non2error` 재접속 후 같은 거래 `COMMITTED/시도 1회/체크포인트 0`, 실제 곡괭이 총 2개·목재 44·섬유 46을 확인해 통과했다. 구 JAR의 동결 시계에서는 `/ws test inspect transaction`이 이전 거래를 최신으로 잘못 보여줬지만 `fault status`의 예약 ID와 저장 파일은 일치했다. 로컬 `4207557`은 삽입 순서 기준 최신 선택 및 동시각 회귀 테스트를 추가했고 53 suite·189 test 무실패, 후보 JAR SHA-256 `0F4886B32B71CA924667F666CA03111D17C693652DDF41DFBE0F57A9A5678CB9`다. 동일 빌드 `PROCESSING`·`COMMITTED` 종료 검증 전에는 후보를 실서버에 배포하지 않는다.

`P2-CRAFT-PROCESSING-KILL-002`는 세 번째 급조 곡괭이 거래가 `PROCESSING` 내구 저장 직후 강제 종료됐을 때 접속 전 같은 거래·개인 목재 41·섬유 44·시도 1회를 보존했다. 동일 JAR 재시작·`non2error` 접속에서 `COMMITTED/체크포인트 0`으로 전이하고 실제 곡괭이 총 3개·목재 41·섬유 44로 수렴해 통과했다. `CRAFT` 거래별 outbox는 정확히 1개이며, `COMMITTED` 실물 지급 직전 장애 검사는 아직 미완료다.

`P2-CRAFT-COMMITTED-PAUSE-DEFECT-003`은 네 번째 거래가 `pausedPhase=COMMITTED`였지만 인벤토리 다음 틱의 일반 등록 아이템 flush가 보류 출력을 지급해, JVM 종료 전 실제 곡괭이 총 4개·대기 지급 0으로 끝난 결함이다. 실물 지급 전 중단점 통과로 인정하지 않고 실제 JVM도 종료하지 않았다. 게이트는 `/ws test fault clear`로 해제했다. 로컬 `2af4a3a`은 등록 아이템·자원·장비 인스턴스의 모든 별도 물리 복구 경로가 테스트 일시 정지된 제작 출력만 보류하도록 했고 54 suite·191 test 무실패, 후보 JAR SHA-256 `80CDD6EF44E1E275BAB0BD393326B1F15632419877EB2F2934D0D96281FA31C6`를 만들었다. export·로그아웃·안전 배포 뒤 같은 `COMMITTED` 중단점을 다시 실행하기 전까지 P2의 장애 복구 게이트는 열려 있다.

실클라이언트 로그아웃과 `test-lab-1789449833233.json` export를 확인한 뒤 격리 Paper 서버를 정상 종료했다. 구 JAR을 `wsplugin-old-2C13E408.jar`로 백업·해시 대조하고 후보 `80CDD6EF44E1E275BAB0BD393326B1F15632419877EB2F2934D0D96281FA31C6`로 정확히 한 플러그인 파일만 교체·대조했다. 서버는 wsplugin 1개와 기존 Test Lab 회차를 정상 복원했고 70개 콘텐츠 파일·334개 도감·21개 상태를 로드했다. 이 배포는 런타임 검증 환경 준비이며 `COMMITTED` 보류·실제 JVM 종료·재접속 지급의 통과 증거는 아니다.
