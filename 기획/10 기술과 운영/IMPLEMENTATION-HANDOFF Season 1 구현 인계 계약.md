...
P2 해금 입력은 `non2error`의 실제 GUI에서 성공 문구·원목 잔량 0·3×3 칸 개방까지 통과했다. 회차의 해금 키는 1개이고 실물 원목 체크포인트는 비었다. 같은 회차의 `CRAFT` 시나리오에서 급조 곡괭이 고정 조합과 실제 1개 지급, 목재 `50→47`·섬유 `50→48`, 단일 `CRAFT/COMMITTED` 거래 및 미완료 체크포인트 0을 확인했다. 구 배포 JAR의 해금·제작 성공 통계 중복은 각각 outbox 1개와 별개이며, 로컬 `3cda3d3`은 `CRAFT_UNLOCKED`와 `CRAFT_COMMITTED`를 내구 커밋 outbox에서만 발행하고 후자에 거래 ID를 싣는다. 자동 검증은 52 suite·188 test 무실패, 새 로컬 JAR SHA-256은 `3AB1F24B065AF1A9C5007D10E76C6590EE7FAE5EAA1E59818F790AC4C72D50C1`다. 이 후보는 접속 중인 격리 서버에 아직 배포하지 않았으며, P2 전체 게이트는 GUI 종료 반환·제작 중단 복구·무보정 Day 진입과 다인 루프 전까지 열린다.
