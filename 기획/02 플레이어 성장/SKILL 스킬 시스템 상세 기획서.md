...
| 유형 | 기본 소비 시점 |
|---|---|
| 기본 공격 | `SKILL-LIST-001 §2.1` 무기별 AP를 실행 확정 시 소비 |
| 즉시 액티브 | 실행 확정 |
| 시전 | 시전 시작 |
| 채널링 | 시작 비용과 주기 비용 |
| 토글 | 활성화 비용과 유지 비용 |
...
두 입력 후보 중 `무기 제스처 + 공용 액티브 핫바 + 퀵 아이템 벨트` 하이브리드를 채택한다. 모든 스킬을 핫바 1~7에 두는 안은 소모품·음식·상태 대응 아이템을 즉시 선택할 공간이 한 칸만 남고, 휠 스크롤 중 스킬 오발 위험이 커서 기본안으로 사용하지 않는다.

| 입력·슬롯 | 내부 바인드 | 기본 역할 |
|---|---|---|
| 슬롯 `0`, 좌클릭 `L` | `BASIC_L` | 장착 무기의 기본 공격 스킬. 권투는 가상 기본 공격 프로필 |
| 슬롯 `0`, 우클릭 `R` | `WEAPON_1_R` | 문맥 상호작용이 없을 때 무기 액티브 `W1` |
| 슬롯 `0`, 웅크리기+좌클릭 | `WEAPON_2_SHIFT_L` | 무기 액티브 `W2` |
| 슬롯 `0`, 웅크리기+우클릭 | `WEAPON_3_SHIFT_R` | 무기 액티브 `W3` |
| 슬롯 `0`, 손 바꾸기 키 `F` | `OFFHAND_F` | 방패의 짧은 패링·단기 방어 또는 보조무기 행동, 실제 손 교환 취소 |
| 슬롯 `0`, `Shift+2~5` | `COMMON_1~4` | 공용 액티브 최대 4개 |
| 슬롯 `0`, `Shift+6~9` | `QUICK_ITEM_1~4` | GUI 바인딩 커스텀 소모품 최대 4개 |
| 일반 `2~9`, 슬롯 `1~8` | `VANILLA_HOTBAR` | 바닐라 자유 슬롯 선택·채굴·설치·섭취 |
| 웅크리기 두 번 | `DODGE_DOUBLE_SNEAK` | 기본 회피 |
| 웅크리기+점프 | `DODGE_SNEAK_JUMP` | 대체 회피 |

기본 공격은 무기 액티브 3칸을 차지하지 않는다. 무기 액티브는 해금된 후보 중 최대 3개, 공용 액티브는 해금된 후보 중 최대 4개를 장착한다.

### 25.2 기본 공격 분류

- 검·도끼·활·석궁·단검·둔기·지팡이·곡괭이·삼지창의 좌클릭은 바닐라 공격 결과를 취소하고 `BASIC_ATTACK` 스킬 실행으로 변환한다.
- 장착 무기의 `instanceId`, `weaponClass`, 기본 공격 스킬 ID와 데이터 리비전을 실행 스냅샷에 넣는다.
- 기본 공격 AP·간격·내구·탄약은 `SKILL-LIST-001 §2.1`의 10개 ID가 권위다. 기본 공격은 `SILENCE`로 막히지 않지만 빈사·기절·무장 해제·공격 불가 상태는 정상 적용한다.
- 활·석궁·삼지창을 포함해 바닐라 투사체 피해·장전 피해·투척 피해를 실제 결과로 사용하지 않는다.
- 권투는 아이템에 귀속된 기본 공격 스킬을 만들지 않고 `UNARMED_COMBAT` 가상 프로필의 콤보 실행기를 유지한다. 다만 바닐라 피해는 다른 무기와 마찬가지로 취소하고 최종 피해는 공통 Combat 파이프라인이 확정한다.

### 25.3 무기 액티브 바인드

- `W1~W3`은 커스텀 로드아웃 GUI에서만 배치·교체한다.
- `W2/W3`은 누름형·토글형만 허용한다. 입력 해제를 요구하는 `HOLD_RELEASE` 스킬은 우클릭 입력을 받는 `W1`에만 배치할 수 있다.
- 일반 `F` 입력은 `PlayerSwapHandItemsEvent`를 취소한 뒤 `OFFHAND_F`로 변환한다. 실제 슬롯 `0/-106` 아이템은 바뀌지 않는다. 방패는 키 해제 감지·홀드·토글 없이 `1틱 시작 → 4틱 패링 → 12틱 단기 방어 → 자동 종료`로 실행한다.
- 방패 단기 방어는 시간 비례 AP를 소비하지 않고 방어한 공격의 가드 충격 AP만 소비한다. 기본 공격·스킬·회피·슬롯 이탈·방패 해제·강한 행동 불가 상태는 즉시 취소한다.
- `Shift+F`는 플레이어 메뉴 전용이며 `OFFHAND_F`를 발행하지 않는다. 슬롯 `1~8`의 일반 `F`도 서버 권위 보조무기 `-106`과의 클라이언트 고스트 복제를 막기 위해 손 교환을 취소한다. 슬롯 `1~8`의 `L/R` 바닐라 행동에는 영향을 주지 않는다.
- 유효한 빈사 아군·시설·구조 대상에 대한 문맥 상호작용은 `R`과 `Shift+R`보다 우선한다. 이 경우 충돌하는 W1/W3은 실행되지 않는다.
- 장착되지 않은 바인드는 비용·쿨다운 없이 `SKILL_BIND_EMPTY`만 표시하며 다른 스킬로 자동 대체하지 않는다.

### 25.4 공용 액티브 숫자 조합

- held-slot 변경 직전 선택 슬롯이 0이고 플레이어가 웅크린 상태에서 표시 키 2~5를 선택할 때만 C1~C4로 해석한다.
- 입력 전 슬롯이 1~8이면 같은 조합을 가로채지 않는다. 블록 배치·잠입·바닐라 아이템 선택을 유지한다.
- 공용 액티브 입력을 받으면 목적 슬롯 전환을 취소하고 스킬 ID를 스냅샷한다. 성공·검증 실패와 관계없이 다음 틱 슬롯 0을 확정한다.
- 서버가 발생시킨 복귀 held-slot 이벤트는 `inputSequence`로 무시해 재귀 발동을 막는다.
- 대상·AP·쿨다운 검증 실패는 비용을 소비하지 않으며 같은 실패 알림은 0.5초에 한 번만 표시한다.
- 같은 슬롯 번호 재입력은 Bukkit 이벤트가 오지 않을 수 있으나 전투 자세는 slot 0이므로 C/Q 조합은 항상 다른 목적 번호를 선택한다. 별도 패킷 플러그인을 필수 의존성으로 두지 않는다.

### 25.5 퀵 아이템 벨트

- 퀵 아이템은 `/ws equipment` GUI의 Q1~Q4에 item ID를 바인딩하며 실제 핫바 5~8을 점유하지 않는다.
- 입력 전 slot 0에서 `Shift+6~9`를 누르면 바인딩 아이템을 개인 인벤토리/개인 소모품 원장에서 검증한다. 즉시형은 수량·사용 횟수·지속 가능한 효과를 한 번에 커밋하고, 야전 배급팩은 완료 틱에 다시 검증한 뒤 1개를 소비해 취소·접속 종료 시 수량을 잃지 않는다.
- 성공·검증 실패·채널 취소 뒤 slot 0을 확정한다. 채널 중 좌클릭은 사용을 취소할 뿐 같은 입력의 기본 공격을 자동 연쇄하지 않는다.
- 공용 액티브가 붕대·정화 앰풀 같은 아이템을 소비하더라도 같은 개인 원장을 사용한다. 공용 스킬과 퀵 아이템이 동시에 같은 마지막 1개를 소비할 수 없다.
- 자동 보충을 켠 슬롯은 성공적인 소비 커밋 뒤 개인 인벤토리의 동일 `itemId` 스택에서만 보충한다. 바닥·상자·파티 보관함에서는 가져오지 않는다.
- 퀵 아이템 등록·순서 변경·자동 보충 설정은 비전투 커스텀 GUI에서만 수행한다.

### 25.6 월드 입력 우선순위

```text
열린 GUI·채팅·관전·행동 불가
→ 빈사 구조·시설·등록 개체 문맥 상호작용
→ F 보조무기·방패 행동
→ R/Shift+L/Shift+R W1/W2/W3 무기 액티브
→ C1~C4 공용 액티브
→ Q1~Q4 바인딩 소모품
→ 장착 무기 BASIC_ATTACK 또는 권투 가상 기본 공격
→ 허용된 바닐라 블록·아이템 상호작용
```

- 곡괭이는 좌클릭 시 공격 가능한 개체를 먼저 검사하고, 없을 때만 허용된 블록 채굴로 내려간다.
- 권투 상태에서 허용 블록을 조준했고 공격 대상이 없으면 웅크리기+좌클릭의 블록 파괴를 우선하며, 그 외에는 W2를 실행한다.
- 오른쪽 입력은 slot 0에서만 W1/W3으로 사용한다. slot 1~8에서는 음식·소모품·문·상자·블록 등 바닐라 우클릭을 보존한다.
...
- 무기 `W1~W3`, 공용 `C1~C4` 외 자유 액티브 슬롯은 제공하지 않는다.
...
players:
  "player-uuid":
    skill-loadout:
      weapon-actives:
        W1: player.staff.arc_bolt.v1
        W2: player.staff.ward_field.v1
        W3: null
      common-actives:
        C1: ws.common.field_bandage.v1
        C2: ws.common.threat_ping.v1
        C3: null
        C4: null
      combat-hotkey-mode: SLOT_ZERO_SHIFT_NUMBER
    quick-items:
      Q1: { item-id: WSI-CONS-BANDAGE, auto-refill: true }
      Q2: { item-id: WSI-CONS-PURIFY_AMPOULE, auto-refill: true }
      Q3: null
      Q4: null
    cooldowns:
      player.staff.arc_bolt.v1:
...
- 장착 무기 기본 공격 ID는 skill-loadout에 중복 저장하지 않고 `MAIN_WEAPON` 인스턴스와 잠긴 콘텐츠 리비전에서 파생한다.
- 권투 기본 공격은 빈 `MAIN_WEAPON` 상태에서 파생하며 장착 스킬 ID로 직렬화하지 않는다.
- 로드아웃은 스킬 ID와 바인드만 저장하고 핫바 프록시 ItemStack 자체는 저장하지 않는다.
...
| 장착 무기 기본 공격 | 바닐라 피해 취소·BASIC_ATTACK 1회·액티브 미점유 검증 |
| W1~W3·C1~C4 | 바인드 호환·슬롯 0 복귀·복귀 이벤트 재귀 차단 검증 |
| SLOT_ZERO_SHIFT_NUMBER | 입력 전 slot 0 전용·일반 2~9·slot 1~8 바닐라 보존 |
| Q1~Q4 아이템 경합 | 공용 액티브와 마지막 1개 동시 소비·자동 보충 검증 |
...
- 장착 무기 기본 공격, 권투 가상 기본 공격, W1~W3와 C1~C4가 서로 다른 소유·슬롯 계약으로 실행된다.
- 공용 액티브와 Q1~Q4 소모품이 같은 개인 원장에서 원자적으로 소비된다.
...
- 몬스터 등급, 종족, 속성, 보스 페이즈, 상태이상 보유 여부, 활성 시설망 범위, 오염 구간을 조건으로 사용할 수 있다.
- 시전자 자신은 기본 제외하지만 `include-self: true`로 스킬별 허용한다.
- 공격 스킬은 기본 모드에서 아군을 제외한다. 선택형 전술 모드만 `CORE-001`의 제한된 아군 피해, 브레이크와 강제 이동 배율을 따른다.
...
- WildSurvival의 BigNumber 피해, DEF, PEN, 회차별 아군 피해 정책, 빈사, 브레이크와 기여도를 알지 못한다.
...
- 회피, 패링, 회차별 아군 피해 정책, 강제 이동과 브레이크
...
후속 작업과 구현 게이트는 `ROADMAP-001`, 직접 위임과 문서 상태는 `DOC-AUTHORITY-001`을 따른다.
