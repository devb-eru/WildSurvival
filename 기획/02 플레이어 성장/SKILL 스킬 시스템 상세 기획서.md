...
| 슬롯 `0`, 웅크리기+좌클릭 | `WEAPON_1_SHIFT_L` | 무기 액티브 `W1` |
| 슬롯 `0`, 웅크리기+우클릭 | `WEAPON_2_SHIFT_R` | 무기 액티브 `W2` |
| 슬롯 `0`, 손 바꾸기 키 `F` | `WEAPON_3_SWAP` | 무기 액티브 `W3`, 실제 손 교환은 취소 |
| 슬롯 `0`, 우클릭 `R` | `CONTEXT_OR_OFFHAND_R` | 문맥 상호작용·소모품·보조무기·방패 방어 |
| 핫바 `1~4`, 표시 키 `2~5` | `COMMON_1~4` | 공용 액티브 최대 4개 |
| 핫바 `5~8`, 표시 키 `6~9` | `QUICK_ITEM_1~4` | 소모품·음식·허용 유틸리티 최대 4개 |
...
- 기본 공격은 AP 0이며 `SILENCE`로 막히지 않지만, 빈사·기절·무장 해제·공격 불가 상태는 정상 적용한다.
...
- `W1`과 `W3`은 누름형·토글형만 허용한다. 입력 해제를 요구하는 `HOLD_RELEASE` 스킬은 우클릭 입력을 받는 `W2`에만 배치할 수 있다.
- `F` 입력은 `PlayerSwapHandItemsEvent`를 취소한 뒤 `W3` 명령으로 변환한다. 실제 슬롯 `0/-106` 아이템은 바뀌지 않는다.
- 유효한 빈사 아군·시설·구조 대상에 대한 문맥 상호작용은 `Shift+R`보다 우선한다. 이 경우 `W2`는 실행되지 않는다.
- 장착되지 않은 바인드는 비용·쿨다운 없이 `SKILL_BIND_EMPTY`만 표시하며 다른 스킬로 자동 대체하지 않는다.

### 25.4 공용 액티브 핫바 입력

Paper의 held-slot 변경 이벤트는 이전·새 슬롯만 제공하며 숫자키와 휠 스크롤의 원인을 구분하지 않는다. 따라서 기본 모드는 `SNEAK_SELECT`로 고정한다.

| 모드 | 발동 | 용도 |
|---|---|---|
| `SNEAK_SELECT` | 웅크린 상태로 표시 키 `2~5` 선택 | 기본값, 스크롤 오발 방지 |
| `SELECT_CONFIRM` | 슬롯 `1~4` 선택 후 좌클릭으로 확정 | 접근성 대안, 한 단계 느리지만 오발 방지 |
| `DIRECT_SELECT` | 슬롯 `1~4` 선택 즉시 발동 | 개인 명시 동의가 필요한 고급 설정 |

- `DIRECT_SELECT` 설정 화면에는 숫자키와 휠을 구분할 수 없어 스크롤로도 발동할 수 있음을 경고한다.
- 공용 액티브 입력을 받으면 해당 슬롯의 스킬 ID를 스냅샷하고, 성공·실패와 관계없이 다음 틱에 선택 칸을 슬롯 `0`으로 복원한다.
- 복원으로 발생한 held-slot 이벤트는 서버 발급 `inputSequence`로 무시해 재귀 발동을 막는다.
- 대상·AP·쿨다운 검증 실패는 비용을 소비하지 않고 슬롯 `0`으로 복귀하며, 같은 실패 알림은 0.5초에 한 번만 표시한다.
- 웅크린 슬롯 선택이 공용 액티브로 인정되면 진행 중인 회피 더블탭 대기 상태를 취소한다.

### 25.5 퀵 아이템 벨트

- 핫바 `5~8`은 실제 사용 가능한 소모품·음식·허용 유틸리티를 최대 4개 둔다. 스킬 프록시를 배치하지 않는다.
- 퀵 아이템은 표시 키 `6~9`로 선택하고 우클릭으로 사용한다. 즉시형은 커밋 다음 틱, 채널형은 성공·취소 후 슬롯 `0`으로 복귀한다.
- 퀵 아이템 사용 중 좌클릭하면 채널 불가 상태가 아닐 때 아이템 사용을 취소하고 슬롯 `0`으로 복귀한 뒤 같은 입력으로 기본 공격을 한 번 시도한다.
...
- 권투 상태에서 비전투 블록을 조준한 `Shift+L`은 의도적 블록 파괴를 우선하며, 그 외에는 `W1`을 실행한다.
- 오른쪽 입력을 무기 스킬 전용으로 사용하지 않는 이유는 방패 유지, 음식·소모품, 문·상자·시설과 빈사 구조를 같은 바닐라 입력으로 보존하기 위해서다.
  ...
  "player-uuid":
    skill-loadout:
      ...
      common-actives:
        ...
        C3: null
        C4: null
      common-input-mode: SNEAK_SELECT
    quick-items:
      Q1: { item-id: WSR-BANDAGE, auto-refill: true }
      Q2: { item-id: WSR-PURIFY-AMPOULE, auto-refill: true }
      Q3: null
      Q4: null
    cooldowns:
      player.staff.arc_bolt.v1:
...
| SNEAK/CONFIRM/DIRECT | 숫자키·휠·회피 더블탭 충돌 검증 |
