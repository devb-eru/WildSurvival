...
| 충돌 분리 | `Shift+F`는 메뉴, 슬롯 `1~8` 일반 `F`는 고정 보조무기 보호를 위해 손 교환 취소·안내 |

이 결정은 `ADR-001~002`의 확정값과 독립적이다. 구현 권위는 `COMBAT-004`, 입력 권위는 `SKILL-001 §25`와 `IMPLEMENTATION-HANDOFF-001`을 따른다.

2026-08-25 실클라이언트 검증에서 슬롯 `1~8`의 바닐라 손 교환과 서버 권위 `-106` 복구가 충돌해 같은 방패가 두 위치에 보이는 클라이언트 고스트가 재현되었다. 서버 원장과 엔티티 NBT에는 같은 인스턴스 1개만 존재했으나 입력 결과가 오인과 반복 동기화를 유발하므로, `F`의 실제 손 교환은 모든 슬롯에서 취소하는 것으로 보정했다. 이 보정은 슬롯 `1~8`의 `L/R` 바닐라 자유 행동을 변경하지 않는다.
