...
| 선택 | 진행 방식 | 검토 부담 |
|---|---|---|
| `ASSISTANT_DRAFT` 권장 | 기존 Story 비트·대표 문장만 근거로 82개 payload 초안을 작성하고 사용자가 최종 승인 | 한 번의 전체 톤·사실 검토 |
| `CHAPTER_REVIEW` | 프롤로그→1~5장→Final 순서로 장별 초안·승인을 반복 | 가장 안전하지만 승인 횟수 7회 이상 |
