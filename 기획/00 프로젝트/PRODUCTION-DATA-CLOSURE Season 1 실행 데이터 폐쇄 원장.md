...
현재 61개 아이템과 214개 장비에는 완전 사망 플레이어를 되살리는 권위 효과가 없다. 2026-08-24 사용자 권위로 `REVIVAL_ITEM`을 선택했으며, 실제 신규 ID·레시피·도감 반영은 현재 중단 뒤 `NEXT-GOAL-S1-002`에서 수행한다.
...
목록과 런타임 반영 전에는 기존 `DEAD`·유품·관전만 유지하며 가짜 부활 버튼이나 관리자 명령을 정상 플레이 경로로 노출하지 않는다.
