...
| 실행 대상 | 기획 입력 `s1-plan-20260824-r1` → `ws-content-r2.1`, `S1_dev`; 기존 r2 불변 |
| 최종 수정일 | 2026-08-24 |
...
| 비장비 아이템 | `ITEM-LIST-001` | 62 | 보유·사용·거부·환불·도감 | `PLANNED_62/RUNTIME_61` |
...
| 레시피 | `RECIPE-LIST-001` | 316 | 3×3 일치·수량·태그·증명·원자 출력 | `PLANNED_316/RUNTIME_315` |
...
| 적·보스·지원 개체 | `ENTITY-LIST-001` | 92 | 생성·행동·수명·보상 권위·cleanup | `PLANNED_92/RUNTIME_91` |
...
2026-08-24 사용자 권위로 `REVIVAL_ITEM`을 선택하고 `WSI-CONS-REVIVAL_CORE`, `WSRCP-D31-S01`, codex 0118을 ITEM/RECIPE/DEATH 권위에 반영했다. 기획 고정 수는 아이템 62·레시피 316·도감 335이며 현재 r2 61·315·334와의 차이는 P0.5 투영 게이트가 소유한다.
...
런타임 반영 전에는 기존 `DEAD`·유품·관전만 유지하며 가짜 부활 버튼이나 관리자 명령을 정상 플레이 경로로 노출하지 않는다.

### 8.2 Story 원문

`STORY-DATA-001`의 장면 73개·선택 기록 9개 ID/트리거는 유지한다. 작성 방식은 `CHAPTER_REVIEW`로 사용자 잠금됐지만, 한국어 대사·로그 본문·화자 말투·선택지 최종 문구 작성은 전체 플레이 가능 판정 뒤 최종 단계까지 연기한다. 그전에는 ID와 요약 키만 표시하는 개발 폴백을 허용하되 출시 완료로 인정하지 않는다.

## 9. 실제 플레이 전환 순서

| 순서 | 작업 묶음 | 진입 조건 | 종료 증거 |
|---:|---|---|---|
| 1 | r2.1 스키마·생성기 데이터 폐쇄 | `s1-plan-20260824-r1` 승인 | 자연어 추론 0, L0 참조·수량·경계 전수 통과 |
| 2 | 개인/공용 자원·제작·도감·메뉴 | 1 | FAC-S16 전후 원장, 316 제작식, 335 고정 도감, Shift+F 실클라 E2E |
| 3 | 아이템·탄약·장비·도구 | 2 | 62+214 ID의 사용/장착/파손/복구 상태 원장 전부 `VERIFIED` |
| 4 | 스킬·개인/파티 증강·상태 | 3 | 64+66+21 ID의 효과·상충·저장·인원 행렬 통과 |
| 5 | 적·지원 개체·드롭·Day 사건 | 4 | 92 엔티티·69 적 행동·62 loot·Day1~50 생성/cleanup/보상 통과 |
| 6 | 시설·연구 | 2~5의 의존 기능 | 46 opcode와 연구25 노드 정상/거부/중단/재시작 통과 |
| 7 | 보스·Final 기계 완성 | 4~6, `REVIVAL_ITEM` 데이터 | Day10/20/30/40, Day50 TX, 개발 폴백으로 전체 완주 |
| 8 | Story 최종 작성 | 전체 플레이 가능 판정 | `CHAPTER_REVIEW`, 원문 표현 E2E |
